...
- 数字化转型最终是服务于客户体验的持续改进， 技术保障部不面向最终客户需求， 但不同团队输出成果的使用方便是客户， 支撑目的是解决用户使用问题， 用户问题的量化是有的，但数字化的方式的统计分析还是不够， 比如前述的跟用例的对比， 检视点数据/账单对比分析等需要加强。自动化测试平台的使用对象是自动化测试团队，自动化测试团队就需要定期提供量化的反馈给运维开发团队。 运维部署上线
